...
static GtkWidget* GTKUI_CreateOpenFileDialog(const char* title)
{
    GtkWidget* dialog = gtk_file_chooser_dialog_new(title, NULL, GTK_FILE_CHOOSER_ACTION_OPEN, "_Cancel", GTK_RESPONSE_CANCEL, "_Open", GTK_RESPONSE_ACCEPT, NULL);
    return dialog;
}

static GtkWidget* GTKUI_CreateSaveFileDialog(const char* title)
{
    GtkWidget* dialog = gtk_file_chooser_dialog_new(title, NULL, GTK_FILE_CHOOSER_ACTION_SAVE, "_Cancel", GTK_RESPONSE_CANCEL, "_Save", GTK_RESPONSE_ACCEPT, NULL);
    return dialog;
}
...
static GtkWidget* GTKUI_LayoutWindow()
{
    ...
    GtkWidget* window = GTKUI_CreateVerticalLayout(&topBar, 1, &image, 1);
