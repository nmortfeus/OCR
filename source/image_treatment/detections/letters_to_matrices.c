...
void print_matrix(int *matrix) {
    ...
    for (int i = 0; i < MATRIX_SIZE; i++) {
        for (int j = 0; j < MATRIX_SIZE; j++) {
            printf("%i", matrix[i * MATRIX_SIZE + j]);
        }
        printf("\n");
    }
    printf("\n------------------------------------------------------------------------------------------\n");
...
void letters_to_matrices(Letter *letters, int *grid_letters_index, int nb_grid_letters, int *list_letters_index, int nb_list_letters, int ***grid_letters_matrices, int ***list_letters_matrices, int **words_and_grid_nb, int *len_words_etc) {
    *len_words_etc = 0;
    *grid_letters_matrices = malloc((nb_grid_letters + 1)* sizeof(int *));
    ...
    for (int i = 0; i < nb_grid_letters; i++) {
        ...
        //print_matrix(matrix);
        //write_list_to_file("output2.txt", matrix, MATRIX_SIZE*MATRIX_SIZE);
        (*grid_letters_matrices)[i] = matrix;
    }
    (*grid_letters_matrices)[nb_grid_letters] = (int *)NULL;

    
    *list_letters_matrices = malloc((nb_list_letters + 1) * sizeof(int *));
    ...
    (*list_letters_matrices)[nb_list_letters] = (int *)NULL;
